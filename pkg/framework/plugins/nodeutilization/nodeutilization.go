...
func assessNodesUsagesAndStaticThresholds(
	...
	// we ensure that the user provided thresholds are valid here.
	lowSpan, highSpan = sanitizeUserProvidedThresholds(lowSpan, highSpan)
...
// sanitizeUserThresholds ensures that the user provided thresholds are valid.
// If both low and high thresholds are 0 for a given resource this function
// makes them both 100% to avoid making all nodes overutilized and
// underutilized at the same time.
func sanitizeUserProvidedThresholds(
	low, high api.ResourceThresholds,
) (api.ResourceThresholds, api.ResourceThresholds) {
	newLow := api.ResourceThresholds{}
	newHigh := api.ResourceThresholds{}
	for rname, lowValue := range low {
		highValue := high[rname]
		if lowValue == 0 && highValue == 0 {
			lowValue, highValue = 100, 100
		}
		newLow[rname] = lowValue

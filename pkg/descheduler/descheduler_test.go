...
func initDescheduler(t *testing.T, ctx context.Context, internalDeschedulerPolicy *api.DeschedulerPolicy, assumedRequestTimeout uint, objects ...runtime.Object) (*options.DeschedulerServer, *descheduler, *fakeclientset.Clientset) {
	...
	descheduler, err := newDescheduler(ctx, rs, internalDeschedulerPolicy, "v1", eventRecorder, sharedInformerFactory, assumedRequestTimeout)
	if err != nil {
...
func TestPodEvictorReset(t *testing.T) {
	...
	rs, descheduler, client := initDescheduler(t, ctxCancel, internalDeschedulerPolicy, evictions.AssumedEvictionRequestTimeoutSeconds, node1, node2, p1, p2)
	defer cancel()
...
func TestEvictionRequestsCache(t *testing.T) {
	...
	internalDeschedulerPolicy := removePodsViolatingNodeTaintsPolicy()
	ctxCancel, cancel := context.WithCancel(ctx)
	_, descheduler, client := initDescheduler(t, ctxCancel, internalDeschedulerPolicy, evictions.AssumedEvictionRequestTimeoutSeconds, node1, node2, p1, p2, p3, p4)
	defer cancel()
...
func TestDeschedulingLimits(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			policy: func() *api.DeschedulerPolicy {
				policy := removePodsViolatingNodeTaintsPolicy()
				policy.MaxNoOfPodsToEvictTotal = pointer.Uint(4)
				return policy
			}(),
			limit: uint(4),
		},
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			...
			nodes := []*v1.Node{node1, node2}
			ctxCancel, cancel := context.WithCancel(ctx)
			_, descheduler, client := initDescheduler(t, ctxCancel, tc.policy, evictions.AssumedEvictionRequestTimeoutSeconds, node1, node2) //, p1, p2, p3, p4, p5)
			defer cancel()
...
func TestEvictionRequestsCacheCleanup(t *testing.T) {
	...
	internalDeschedulerPolicy := removePodsViolatingNodeTaintsPolicy()
	ctxCancel, cancel := context.WithCancel(ctx)
	_, descheduler, client := initDescheduler(t, ctxCancel, internalDeschedulerPolicy, 1, node1, node2, p1, p2, p3, p4)
	defer cancel()

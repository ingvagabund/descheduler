...
func initDescheduler(t *testing.T, ctx context.Context, internalDeschedulerPolicy *api.DeschedulerPolicy, metricsClient metricsclient.Interface, objects ...runtime.Object) (*options.DeschedulerServer, *descheduler, *fakeclientset.Clientset) {
...
func TestPodEvictorReset(t *testing.T) {
	...
	rs, descheduler, client := initDescheduler(t, ctxCancel, internalDeschedulerPolicy, nil, node1, node2, p1, p2)
	defer cancel()
...
func TestDeschedulingLimits(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			...
			nodes := []*v1.Node{node1, node2}
			ctxCancel, cancel := context.WithCancel(ctx)
			_, descheduler, client := initDescheduler(t, ctxCancel, tc.policy, nil, node1, node2)
			defer cancel()

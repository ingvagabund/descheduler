...
func NewPodEvictor(
	...
	erCache := newEvictionRequestsCache(assumedRequestTimeout)
...
func (pe *PodEvictor) NodeLimitExceeded(node *v1.Node) bool {
	...
	if pe.maxPodsToEvictPerNode != nil {
		return pe.nodepodCount[node.Name]+pe.erCache.evictionRequestsPerNode(node.Name) >= *pe.maxPodsToEvictPerNode
	}
	return false
}
...
func (pe *PodEvictor) EvictPod(ctx context.Context, pod *v1.Pod, opts EvictOptions) bool {
	...
	ignore, err := pe.evictPod(ctx, pe.client, pod, pe.policyGroupVersion)
	if err != nil {
...
// return (ignore, err)
func (pe *PodEvictor) evictPod(ctx context.Context, client clientset.Interface, pod *v1.Pod, policyGroupVersion string) (bool, error) {
